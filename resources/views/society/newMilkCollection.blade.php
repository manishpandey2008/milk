			...
			<div class="container-fluid home ">
				...
				<div class="container-fluid widget-area proclinic-box-shadow  my-3">
					...
					<div style="width:100%;height: 2px;background-color: black"></div>
						...
						<div class="row">
							<div class="col-lg-6 col-sm-12 row mx-auto my-3 " id="serch_farmer">
								<div class="col-8">
							...
							<div class="col-4 mx-auto my-2" style="display: none" id="reset">
								<button class="btn btn-primary"  style="width: 100%">Reset</button>
							</div>
						</div>

					<form id="collectionForm" style="display: none" method="">
		...
		$(document).ready(function(event){
			...
			$(document).on('keyup',function(event){
				 ...
				 if (event.keyCode === 13 && x)
				 {
					...
					$.post('{{ route("getfarmerdata")}}',{'farmer_id':farmer_id,'_token':$('input[name=_token]').val()},function(data){

							$('#collectionForm').show();
							$('#collectionForm').html(data);
			...
			$(document).on('submit','#collectionForm',function(event){
				 event.preventDefault();
				...
				var sendData='{"snf":'+snf+',"clr":'+clr+',"weight":'+weight+',"rate":'+rate+',"farmerId":"'+farmerId+'"}';

				obj = JSON.parse(sendData);
				$.post('{{ route("milkstore")}}',{'data':obj,'_token':$('input[name=_token]').val()},function(data){
						x=1;
    			});

				if (snf && clr && weight) {

					var data2='<tr><td>'+farmerId+'</td><td>'+weight+'</td><td>'+clr+'</td><td>'+snf+'</td><td>'+rate+'</td><td>200</td></tr>';
					
					$('#dataTable').html(data2+data);
