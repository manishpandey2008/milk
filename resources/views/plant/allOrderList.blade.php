		...
		<div id="content">
			...
			<div class="container-fluid home">
				<div class="container-fluid widget-area proclinic-box-shadow  my-3">
					<h4 class="text-center py-3"><strong>Order List</strong></h4>
					...
					<div class="row">
						...
						<div class="col-12 my-2">
							...
							<button style="width: 100%;background-color:rgb(191,255,191);border-radius: 10px;cursor: pointer;" data-toggle="collapse" href="#collapseExample{{$count}}" aria-expanded="false" aria-controls="collapseExample{{$count}}" value="{{$key}}" class="new_order_list_button">
							...
							@else
							<button style="width: 100%;background-color: rgb(255,255,198);border-radius: 10px;cursor: pointer;" data-toggle="collapse" href="#collapseExample{{$count}}" aria-expanded="false" aria-controls="collapseExample{{$count}}" value="{{$key}}" class="new_order_list_button">
								<p class="pt-1 px-2"><span class="float-left"><strong>{{$key}}</strong></span><span class="float-right">{{$value[0]['created_at']}}</span></p>
								<p class="pb-1 text-center"><span><strong>{{$value[0]['order_by_id']}}</strong></span></p>
							</button>
							@endif
	...
	<script type="text/javascript">
		$(document).ready(function(event){
			$(document).on('click','.new_order_list_button',function(event){
				...
				$.post('{{ route("plant.orderlistdata")}}',{'data':val,'_token':$('input[name=_token]').val()},function(data){
