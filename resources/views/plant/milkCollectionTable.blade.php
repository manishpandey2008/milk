...
<body>
					...
					<div class="row">
						<div class="col-12 table-responsive my-4">
							<table class="table text-center table-bordered table-hover table-striped " >
							  ...
							  <tbody>
							  	@foreach($collection_data as $dl)
							  		@php
							  	
							  			$date=explode(" ",$dl['updated_at'])[0];

							  			$time=explode(':',explode(" ",$dl['updated_at'])[1])[0];
							  		@endphp
							  	<tr>
							  	...
							  	<td>{{$dl['milk_weight']}}</td>
							    <td>{{$dl['milk_clr']}}</td>
							    <td>{{$dl['milk_snf']}}</td>
							    <td>{{$dl['milk_rate']}}</td>
							    @else
							    <td>{{$dl['new_milk_weight']}}</td>
							    <td>{{$dl['new_milk_clr']}}</td>
							    <td>{{$dl['new_milk_snf']}}</td>
							    <td>{{$dl['new_milk_rate']}}</td>
							     <td>
							    	<button class="btn btn-primary get_old_data" data-toggle="modal" data-target=".old_data" value="{{$dl['dispatch_id']}}">Old</button>
							    </td>
							    @endif
							    </tr>
							    @endforeach
	...
	<div class="modal fade old_data" tabindex="-1" role="dialog">
	      		...
	      		<div class="row">
	      			<div class="col-lg-6 col-sm-12">
	      				<div class="form-group">
		      				...
		      				<input type="text" name="dispatch_id"  readonly=""  class="form-control" id="dispatch_id">
		      			</div>
	      			</div>
	      			<div class="col-lg-6 col-sm-12">
	      				<div class="form-group">
		      				<label>Old Weight</label>
		      				<input type="text" name="new_weight"  class="form-control" id="new_weight" readonly="">
		      			</div>
	      			</div>
	      			<div class="col-lg-6 col-sm-12">
	      				<div class="form-group">
		      				<label>Old CLR</label>
		      				<input type="text" name="new_clr" readonly=""  class="form-control" id="new_clr">
		      			</div>
	      			</div>
	      			<div class="col-lg-6 col-sm-12">
	      				<div class="form-group">
		      				<label>Old SNF</label>
		      				<input type="text" name="new_snf" readonly="" class="form-control" id="new_snf">
		      			</div>
	      			</div>
	      			<div class="col-lg-6 col-sm-12">
	      				<div class="form-group">
		      				<label>Old Rate</label>
		      				<input type="text" name="new_rate" readonly=""  class="form-control" id="new_rate">
		      			</div>
	      			</div>
	      			<div class="col-lg-6 col-sm-12">
	      				<div class="form-group">
	...
	<script type="text/javascript">
		$(document).ready(function(){
			$(document).on('click','.get_old_data',function(){

				var dispId=$(this).val();
				$.post('{{ route("plant.milk.collection.olddata")}}',{'data':dispId,'_token':$('input[name=_token]').val()},function(data){

					$('#dispatch_id').val(data['dispatch_id']);
					$('#new_weight').val(data['milk_weight']);
					$('#new_clr').val(data['milk_clr']);
					$('#new_snf').val(data['milk_snf']);
					$('#new_rate').val(data['milk_rate']);
